...
class CurrentSong(APIView):
    def get(self, request, format=None):
        ...
        #removing any feat. or with or ft to make it look cleaner
        song_title = re.sub(r'\s*(\([^\)]*\))?\s*\S*\b(feat|with|ft)\S*.*', '', item.get("name")).strip()
...
class GoBack(APIView):
    def post(self, request, format=None):
        ...
        if self.request.session.session_key == room.host:
            ...
            return Response({"Denied": "User is not the host"})
