...
export default class Room extends Component {
  ...
  getAuthLink = () => {
    fetch("/spotify/get-auth-url")
      .then((response) => {
        if (!response.ok) {
          this.setState({ auth_link: "" });
        }
        return response.json();
      })
      .then((data) => {
        this.setState({ auth_link: data.url });
      });
  };
    ...
    return (
      <Grid item xs={12} align="center">
        <Button
          variant="contained"
          color="primary"
          onClick={() => this.onUpdateShowSettings(true)}
        >
          Settings
        </Button>
      </Grid>
      ...
      <div className="main-wrapper">
        <Box position="absolute" top={0} left={0} zIndex={1}>
          ...
          <Typography
            ...
            style={{ textShadow: "1px 1px 2px rgba(0, 0, 0, 0.3)" }}
          >
            {this.state.users.length} Active{" "}
            {this.state.users.length === 0 ? "Users" : "User"}
          </Typography>
          <List>
